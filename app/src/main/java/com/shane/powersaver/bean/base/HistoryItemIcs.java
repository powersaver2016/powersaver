...
public class HistoryItemIcs extends HistoryItem implements Serializable, Parcelable {
    ...
    private HistoryItemIcs(Parcel in) {
        super(in);
    }

    public static final Parcelable.Creator<HistoryItemIcs> CREATOR
            = new Parcelable.Creator<HistoryItemIcs>() {

        @Override
        public HistoryItemIcs createFromParcel(Parcel in) {
            return new HistoryItemIcs(in);
        }

        @Override
        public HistoryItemIcs[] newArray(int size) {
            return new HistoryItemIcs[size];
        }
