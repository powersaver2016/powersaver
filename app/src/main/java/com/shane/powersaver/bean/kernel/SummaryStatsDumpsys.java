...
import com.shane.powersaver.bean.base.Misc;
import com.shane.powersaver.bean.base.StatElement;
import com.shane.powersaver.util.NonRootShell;
...
public class SummaryStatsDumpsys {
     ...
     * @throws Exception
     */
    public static ArrayList<StatElement> getSummaryStats(boolean showWifi, boolean showBt, boolean useRoot) {
        final String START_PATTERN = "Statistics since last charge";
        final String STOP_PATTERN = "Statistics since last unplugged";

        ArrayList<StatElement> mySummary = null;
        long nTotalCount = 0;
        ...
        if ((res != null) && (res.size() != 0)) {
            ...
            mySummary = new ArrayList<StatElement>();
            Misc myMisc = null;
            ...
            for (int i = 0; i < res.size(); i++) {
                ...
                if (bParsing) {
                    ...
                    if (line.find()) {
                        bParsing = true;
                        Log.i(TAG, "line: " + res.get(i));
                    }
                }
            }

            // set the total
            for (int i = 0; i < mySummary.size(); i++) {
                mySummary.get(i).setTotal(total);
            }
        } else {
            mySummary = new ArrayList<StatElement>();
